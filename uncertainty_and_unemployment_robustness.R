...
# порядок переменных в векторной авторегрессии согласно работе [Choi, Loungani, 2015]
# Порядок переменных в векторной авторегрессии аналогичен работе [Choi, Loungani, 2015], но с добавлением промышленного производства: 
# промышленное производство,  уровень безработицы, 
# уровень инфляции, межбанковская ставка процента, индекс неопределенности, 
# совокупная стоимость акций на фондовом рынке.

df_epu_2 <- merge(prod_manuf, regist_unempl_rate)
df_epu_2 <- merge(df_epu_2, infl_rate)
df_epu_2 <- merge(df_epu_2, interest_rate_3m)
df_epu_2 <- merge(df_epu_2, epu_index)
df_epu_2 <- merge(df_epu_2, tot_share_price_grw_rate)
# сохраняем базу данных
write.table(df_epu_2, 'df_epu_2.csv', sep = ',')
# преобразовываем во временные ряды
df_epu_2_ts <- ts(df_epu_2[,-1], start = c(2000, 1), frequency = 12)
# выбор порядка лага
VARselect(df_epu_2_ts)
# формирование системы векторных авторегрессий второго лага
var_epu_2 <- VAR(df_epu_2_ts, 2, 'none')
# выявление причинности по Грейнджеру
causality(var_epu_2, cause = c("epu"), vcov.=vcovHC(var_epu_2))
# шок неопределенности экономической политики на уровень безработицы 
irf_epu_unempl_list_2 <- irf(var_epu_2, impulse = 'epu', response = 'regist_unempl', n.ahead = 24, cumulative = FALSE)
# шок неопределенности экономической политики на капитализацию фондового рынкаы 
irf_epu_tot_share_list_2 <- irf(var_epu_2, impulse = 'epu', response = 'tot_share_price', n.ahead = 24, cumulative = FALSE)
# шок неопределенности экономической политики на межбанковскую процетную ставку 
irf_epu_int_rate_list_2 <- irf(var_epu_2, impulse = 'epu', response = 'int_rate_3m', n.ahead = 24, cumulative = FALSE)
# шок неопределенности экономической политики на уровень инфляции
irf_epu_infation_list_2 <- irf(var_epu_2, impulse = 'epu', response = 'grw_rate_infl', n.ahead = 24, cumulative = FALSE)
# шок неопределенности экономической политики на промышленное производство
irf_epu_prod_manuf_list_2 <- irf(var_epu_2, impulse = 'epu', response = 'product_manuf', n.ahead = 24, cumulative = FALSE)
# 

# ---- volatility of stock market [Bloom, 2009] ----
# total share price of all share
# объединение переменных
df_tot_share_price_2 <- merge(prod_manuf, regist_unempl_rate)
df_tot_share_price_2 <- merge(df_tot_share_price_2, infl_rate)
df_tot_share_price_2 <- merge(df_tot_share_price_2, interest_rate_3m)
df_tot_share_price_2 <- merge(df_tot_share_price_2, tot_share_price_cycle)
df_tot_share_price_2 <- merge(df_tot_share_price_2, tot_share_price_grw_rate)
# сохранение базы данных
write.table(df_tot_share_price_2, 'df_tot_share_price_2.csv', sep = ',')
# преобразование во временные ряды
df_tot_share_price_2_ts <- ts(df_tot_share_price_2[,-1], start = c(2002, 1), frequency = 12)
# определение порядка лага
VARselect(df_tot_share_price_2_ts)
# формирование 
var_tot_share_price_2 <- VAR(df_tot_share_price_2_ts, 2, 'none')
# гипотеза причинности
causality(var_tot_share_price_2, cause = c('cycle_tot_share_price'), vcov. = vcovHC(var_tot_share_price_2))
# шок капитализации на уровень безработицы
irf_tot_share_unempl_list_2 <- irf(var_tot_share_price_2, impulse = 'cycle_tot_share_price', response = 'regist_unempl', n.ahead = 24, cumulative = FALSE)
# шок капитализации на межбанковскую процентную ставку 
irf_tot_share_int_rate_list_2 <- irf(var_tot_share_price_2, impulse = 'cycle_tot_share_price', response = 'int_rate_3m', n.ahead = 24, cumulative = FALSE)
# шок капитализации на уровень безработицы
irf_tot_share_inflation_list_2 <- irf(var_tot_share_price_2, impulse = 'cycle_tot_share_price', response = 'grw_rate_infl', n.ahead = 24, cumulative = FALSE)
# шок капитализации на уровень безработицы
irf_tot_share_prod_manuf_list_2 <- irf(var_tot_share_price_2, impulse = 'cycle_tot_share_price', response = 'product_manuf', n.ahead = 24, cumulative = FALSE)
# шок капитализации на уровень капитализации
irf_tot_share_tot_share_list_2 <- irf(var_tot_share_price_2, impulse = 'cycle_tot_share_price', response = 'tot_share_price', n.ahead = 24, cumulative = FALSE)

# соединение переменных в единую базу данных

df_usd_ruble_2 <- merge(prod_manuf, regist_unempl_rate)
df_usd_ruble_2 <- merge(df_usd_ruble_2, infl_rate)
df_usd_ruble_2 <- merge(df_usd_ruble_2, interest_rate_3m)
df_usd_ruble_2 <- merge(df_usd_ruble_2, usd_to_ruble_cycle)
df_usd_ruble_2 <- merge(df_usd_ruble_2, tot_share_price_grw_rate)
# сохранение массива данных
write.table(df_usd_ruble_2, 'df_usd_ruble_2.csv', sep = ',')
# преобразование во временные ряды
df_usd_ruble_2_ts <- ts(df_usd_ruble_2[,-1], start = c(2000, 2), frequency = 12)
# выбор порядка лага
VARselect(df_usd_ruble_2_ts)
# формирование векторной авторегрессии
var_usd_ruble_2 <- VAR(df_usd_ruble_2_ts, 2, 'none')
# проверка причинности
causality(var_usd_ruble_2, cause = c('usd_ruble_ex_cycle'), vcov. = vcovHC(var_usd_ruble_2))
# шок обменного курса США на уровень безработицы
irf_usd_unempl_list_2 <- irf(var_usd_ruble_2, impulse = 'usd_ruble_ex_cycle', response = 'regist_unempl', n.ahead = 24, cumulative = FALSE)
# шок обменного курса США на межбанковскую процентную ставку
irf_usd_int_rate_list_2 <- irf(var_usd_ruble_2, impulse = 'usd_ruble_ex_cycle', response = 'int_rate_3m', n.ahead = 24, cumulative = FALSE)
# шок обменного курса США на капитализацию фондового рынка
irf_usd_tot_share_list_2 <- irf(var_usd_ruble_2, impulse = 'usd_ruble_ex_cycle', response = 'tot_share_price', n.ahead = 24, cumulative = FALSE)
# шок обменного курса США на промышленное производство
irf_usd_prod_manuf_2 <- irf(var_usd_ruble_2, impulse = 'usd_ruble_ex_cycle', response = 'product_manuf', n.ahead = 24, cumulative = FALSE)
# шок обменного курса США на уровень инфляции
irf_usd_inflation_list_2 <- irf(var_usd_ruble_2, impulse = 'usd_ruble_ex_cycle', response = 'grw_rate_infl', n.ahead = 24, cumulative = FALSE)
...
# уровень занятости (15 лет и старше)
fred_df <- merge(fred_df, empl_rate_all_fred_ts)
...
# соединение переменных в единую базу данных
df_macro_uncert_2 <- merge(prod_manuf, regist_unempl_rate)
df_macro_uncert_2 <- merge(df_macro_uncert_2, infl_rate)
df_macro_uncert_2 <- merge(df_macro_uncert_2, interest_rate_3m)
df_macro_uncert_2 <- merge(df_macro_uncert_2, macro_uncert_measure)
df_macro_uncert_2 <- merge(df_macro_uncert_2, tot_share_price_grw_rate)

# преобразование во временные ряды
df_macro_uncert_2_ts <- ts(df_macro_uncert_2[,-1], start = c(2000, 2), frequency = 12)
# выбор порядка лага
VARselect(df_macro_uncert_2_ts)
# формирование векторной авторегрессии
var_macro_uncert_2 <- VAR(df_macro_uncert_2_ts, 2, 'none')
# проверка причинности и одновременного изменения имсульса и отклика
causality(var_macro_uncert_2, cause = 'uncert_feat', vcov. = vcovHC(var_macro_uncert_2))
# шок макроэкономической неопределенности на уровень безработицы
irf_macro_uncert_unempl_list_2 <- irf(var_macro_uncert_2, impulse = 'uncert_feat', response = 'regist_unempl', n.ahead = 24, cumulative = FALSE)
# шок макроэкономической неопределенности на межбанковскую процентную ставку 
irf_macro_uncert_int_rate_list_2 <- irf(var_macro_uncert_2, impulse = 'uncert_feat', response = 'int_rate_3m', n.ahead = 24, cumulative = FALSE)
# шок макроэкономической неопределенности на уровень инфляции
irf_macro_uncert_inflation_list_2 <- irf(var_macro_uncert_2, impulse = 'uncert_feat', response = 'grw_rate_infl', n.ahead = 24, cumulative = FALSE)
# шок макроэкономической неопределенности на капитализацию фондового рынка
irf_macro_uncert_tot_share_list_2 <- irf(var_macro_uncert_2, impulse = 'uncert_feat', response = 'tot_share_price', n.ahead = 24, cumulative = FALSE)
# шок макроэкономической неопределенности на индекс промышленного производства
irf_macro_uncert_prod_manuf_list_2 <- irf(var_macro_uncert_2, impulse = 'uncert_feat', response = 'product_manuf', n.ahead = 24, cumulative = FALSE)
...
df_pca_all_uncer_feat_2 <- merge(prod_manuf, regist_unempl_rate)
df_pca_all_uncer_feat_2 <- merge(df_pca_all_uncer_feat_2, infl_rate)
df_pca_all_uncer_feat_2 <- merge(df_pca_all_uncer_feat_2, interest_rate_3m)
df_pca_all_uncer_feat_2 <- merge(df_pca_all_uncer_feat_2, uncert_measures_all_pca_values %>% dplyr::select(date, PC1))
df_pca_all_uncer_feat_2 <- merge(df_pca_all_uncer_feat_2, tot_share_price_grw_rate)

# преобразуем во временные ряды
df_pca_all_uncer_feat_2_ts <- ts(df_pca_all_uncer_feat_2[,-1], start = c(2000, 2), frequency = 12)
# выбор порядка лага
VARselect(df_pca_all_uncer_feat_2_ts)
# формирование векторной авторегрессии
var_pca_all_uncer_feat_2 <- VAR(df_pca_all_uncer_feat_2_ts, 2, 'none')
# проверка причинности
causality(var_pca_all_uncer_feat_2, cause = 'PC1', vcov. = vcovHC(var_pca_all_uncer_feat_2))
# шок главной компоненты всех показателей неопределенности на уровень безработицы
irf_pca_unempl_list_2 <- irf(var_pca_all_uncer_feat_2, impulse = 'PC1', response = 'regist_unempl', n.ahead = 24, cumulative = FALSE)
# шок главной компоненты всех показателей неопределенности на капитализацию фондового рынка
irf_pca_tot_share_list_2 <- irf(var_pca_all_uncer_feat_2, impulse = 'PC1', response = 'tot_share_price', n.ahead = 24, cumulative = FALSE) 
# шок главной компоненты всех показателей неопределенности на уровень безработицы
irf_pca_int_rate_list_2 <- irf(var_pca_all_uncer_feat_2, impulse = 'PC1', response = 'int_rate_3m', n.ahead = 24, cumulative = FALSE)
# шок главной компоненты всех показателей неопределенности на уровень инфляции
irf_pca_inflation_list_2 <- irf(var_pca_all_uncer_feat_2, impulse = 'PC1', response = 'grw_rate_infl', n.ahead = 24, cumulative = FALSE)
# шок главной компоненты всех показателей неопределенности на промышленное производство
irf_pca_prod_manuf_list_2 <- irf(var_pca_all_uncer_feat_2, impulse = 'PC1', response = 'product_manuf', n.ahead = 24, cumulative = FALSE)
...
df_irf_unempl_2 <- data.frame(
  time_span = 1:nrow(irf_epu_unempl_list_2$irf$epu),
  epu_unempl = irf_epu_unempl_list_2$irf$epu,
  macro_uncert_unempl = irf_macro_uncert_unempl_list_2$irf$uncert_feat,
  pca_uncert_unempl = irf_pca_unempl_list_2$irf$PC1,
  tot_share_unempl = irf_tot_share_unempl_list_2$irf$cycle_tot_share_price,
  usd_ruble_unempl = irf_usd_unempl_list_2$irf$usd_ruble_ex_cycle
)
# переименовываем метки столбцов
colnames(df_irf_unempl_2) <- c('time_span','epu_unempl','macro_uncert_unempl',
                             'pca_uncert_unempl','tot_share_unempl','usd_ruble_unempl')
# 
write.table(df_irf_unempl_2,'df_irf_unempl_2.csv', sep = ',')

impulse_to_unempl_2 <- df_irf_unempl_2 %>% 
...
df_irf_tot_share_2 <- data.frame(
  time_span = 1:nrow(irf_epu_tot_share_list_2$irf$epu),
  epu_tot_share = irf_epu_tot_share_list_2$irf$epu,
  macro_tot_share = irf_macro_uncert_tot_share_list_2$irf$uncert_feat,
  pca_tot_share = irf_pca_tot_share_list_2$irf$PC1,
  tot_tot_share = irf_tot_share_tot_share_list_2$irf$cycle_tot_share_price,
  usd_tot_share = irf_usd_tot_share_list_2$irf$usd_ruble_ex_cycle
)
# переименовываем столбцы
colnames(df_irf_tot_share_2) <- c('time_span','epu_tot_share',
                                'macro_tot_share','pca_tot_share','tot_share_tot_share',
                                'usd_tot_share')
# 
write.table(df_irf_tot_share_2,'df_irf_tot_share_2.csv', sep = ',')
# 
impulse_to_tot_share_2 <- df_irf_tot_share_2 %>% 
  ggplot(aes(x = time_span)) + 
...
df_irf_int_rate_2 <- data.frame(
  time_span = 1:nrow(irf_epu_int_rate_list_2$irf$epu),
  epu_int_rate = irf_epu_int_rate_list_2$irf$epu,
  macro_int_rate = irf_macro_uncert_int_rate_list_2$irf$uncert_feat,
  pca_int_rate = irf_pca_int_rate_list_2$irf$PC1,
  tot_share_int_rate = irf_tot_share_int_rate_list_2$irf$cycle_tot_share_price,
  usd_int_rate = irf_usd_int_rate_list_2$irf$usd_ruble_ex_cycle
)
# 
colnames(df_irf_int_rate_2) <- c('time_span','epu_int_rate',
                               'macro_int_rate','pca_int_rate','tot_share_int_rate','usd_int_rate')
# 
write.table(df_irf_int_rate_2,'df_irf_int_rate_2.csv', sep = ',')
#
impulse_to_int_rate_2 <- df_irf_int_rate_2 %>% 
  ggplot(aes(x = time_span)) +
...
# (г) уровень инфляции
df_irf_inflation_2 <- data.frame(
  time_span = 1:nrow(irf_epu_infation_list_2$irf$epu),
  epu_inflation = irf_epu_infation_list_2$irf$epu,
  macro_inflation = irf_macro_uncert_inflation_list_2$irf$uncert_feat,
  pca_inflation = irf_pca_inflation_list_2$irf$PC1,
  tot_share_inflation = irf_tot_share_inflation_list_2$irf$cycle_tot_share_price,
  usd_inflation = irf_usd_inflation_list_2$irf$usd_ruble_ex_cycle
)
# 
colnames(df_irf_inflation_2) <- c('time_span','epu_inflation',
                                'macro_inflation','pca_inflation','tot_share_inflation','usd_inflation')
# 
write.table(df_irf_inflation_2,'df_irf_inflation_2.csv', sep = ',')
# 
impulse_to_inflation_2 <- df_irf_inflation_2 %>% 
  ggplot(aes(x = time_span)) +
...
df_irf_prod_manuf_2 <- data.frame(
  time_span = 1:nrow(irf_epu_prod_manuf_list_2$irf$epu),
  epu_prod_manuf = irf_epu_prod_manuf_list_2$irf$epu,
  macro_prod_manuf = irf_macro_uncert_prod_manuf_list_2$irf$uncert_feat,
  pca_prod_manuf = irf_pca_prod_manuf_list_2$irf$PC1,
  tot_share_prod_manuf = irf_tot_share_prod_manuf_list_2$irf$cycle_tot_share_price,
  usd_prod_manuf = irf_usd_prod_manuf_2$irf$usd_ruble_ex_cycle
)
# 
colnames(df_irf_prod_manuf_2) <- c('time_span','epu_prod_manuf',
                                 'macro_prod_manuf','pca_prod_manuf','tot_share_prod_manuf','usd_prod_manuf')
# 
write.table(df_irf_prod_manuf_2,'df_irf_prod_manuf_2.csv', sep = ',')
# 
impulse_to_prod_manuf_2 <- df_irf_prod_manuf_2 %>% 
  ggplot(aes(x = time_span)) +
...
impulse_to_unempl_2
impulse_to_inflation_2
impulse_to_int_rate_2
impulse_to_prod_manuf_2
impulse_to_tot_share_2

fevd_epu_2 <- fevd(var_epu_2, n.ahead = 24)
fevd_macro_2 <- fevd(var_macro_uncert_2, n.ahead = 24)
fevd_tot_share_2 <- fevd(var_tot_share_price_2, n.ahead = 24)
fevd_usd_2 <- fevd(var_usd_ruble_2, n.ahead = 24)
fevd_pca_2 <- fevd(var_pca_all_uncer_feat_2, n.ahead = 24)
