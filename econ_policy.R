...
df_rtsi <- merge(rtsi_cycle, tot_share_price_grw_rate)
df_rtsi <- merge(df_rtsi, interest_rate_3m)
df_rtsi <- merge(df_rtsi, infl_rate)
...
# соединение переменных в единую базу данных
df_bci <- merge(buss_conf_index_cycle, tot_share_price_grw_rate)
df_bci <- merge(df_bci, infl_rate)
...
# соединение переменных в единую базу данных
df_cci <- merge(cons_conf_index_cycle, tot_share_price_grw_rate)
df_cci <- merge(df_cci, infl_rate)
